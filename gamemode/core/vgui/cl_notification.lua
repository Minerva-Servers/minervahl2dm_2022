local gradientRight = minerva.util.GetMaterial("vgui/gradient-l")

local PANEL = {}

function PANEL:Init()
    if ( IsValid( minerva.ui.notificationTab ) ) then
        minerva.ui.notificationTab:Remove()
    end

    minerva.ui.notificationTab = self
...
function PANEL:AddNotification(message, finish, color)
    ...
    surface.PlaySound(minerva.config["notificationSound"])
...
vgui.Register("minerva.NotificationTab", PANEL, "EditablePanel")

if ( IsValid(minerva.ui.notificationTab) ) then
    minerva.ui.notificationTab:Remove()

    vgui.Create("minerva.NotificationTab")
end